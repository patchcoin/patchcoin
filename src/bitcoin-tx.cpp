...
static int AppInitRawTx(int argc, char* argv[])
{
    ...
    if (argc < 2 || HelpRequested(gArgs) || gArgs.IsArgSet("-version")) {
        ...
        std::string strUsage = PACKAGE_NAME " patchcoin-tx utility version " + FormatFullVersion() + "\n";

        if (gArgs.IsArgSet("-version")) {
            strUsage += FormatParagraph(LicenseInfo());
        } else {
            strUsage += "\n"
                "Usage:  patchcoin-tx [options] <hex-tx> [commands]  Update hex-encoded patchcoin transaction\n"
                "or:     patchcoin-tx [options] -create [commands]   Create hex-encoded patchcoin transaction\n"
                "\n";
