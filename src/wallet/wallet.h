...
public:
    ...
    bool CreateCoinStake(ChainstateManager& chainman, const CWallet* pwallet, unsigned int nBits, int64_t nSearchInterval, CMutableTransaction &txNew, std::vector<Claim>& vClaim, CTxDestination destination, CAmount nFees, const uint64_t& nBlockTx);
