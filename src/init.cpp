 ...
 */
static const char* BITCOIN_PID_FILENAME = "patchcoind.pid";
...
void SetupServerArgs(ArgsManager& argsman)
{
    ...
    gArgs.AddArg("-staketargetspacing=<n>", strprintf("Set stake target spacing to <n> (min: (default: %d, testnet: %d, signet: %d))", defaultChainParams->GetConsensus().nStakeTargetSpacing, testnetChainParams->GetConsensus().nStakeTargetSpacing, signetChainParams->GetConsensus().nStakeTargetSpacing), ArgsManager::ALLOW_ANY, OptionsCategory::HIDDEN);
...
bool AppInitMain(NodeContext& node, interfaces::BlockAndHeaderTipInfo* tip_info)
{
    ...
    if (args.IsArgSet("-datadir") && !args.GetPathArg("-datadir").is_absolute()) {
        LogPrintf("Warning: relative datadir option '%s' specified, which will be interpreted relative to the " /* Continued */
                  "current working directory '%s'. This is fragile, because if patchcoin is started in the future "
                  "from a different location, it will be unable to locate the current data files. There could "
                  "also be data loss if patchcoin is started while in a temporary directory.\n",
